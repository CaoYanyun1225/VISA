...
class MergedVisualizationApp:
    def __init__(self, root):
        ...
        style.configure("Accent.TButton", font=("TkDefaultFont", 11))

        # 增加LabelFrame和Label的字体大小
        style.configure("TLabelframe", font=("TkDefaultFont", 11))
        style.configure("TLabelframe.Label", font=("TkDefaultFont", 11))
        ...
        style.configure("TSpinbox", font=("TkDefaultFont", 10))
        ...
        self.attention_data = None  # (sample_numbe*r, shape_number, value_number)
    ...
    def create_basic_controls(self, parent):
        ...
        # 数据信息显示
        self.create_data_info_text(parent)
    ...
    def create_file_loading_section(self, parent):
        ...
        # NPZ文件选择
        ttk.Label(section1, text="Time Series Shapes Data Path:").pack(anchor=tk.W)
        ...
        # NPY文件选择
        ttk.Label(section1, text="Time Series Raw Data Path:").pack(anchor=tk.W, pady=(10, 0))
        ...
        # 加载按钮
        ttk.Button(section1, text="Update Plots", command=self.load_data,
                   style="Large.TButton").pack(fill=tk.X, pady=(5, 0))
    ...
    def create_upper_viz_controls(self, parent):
        ...
        self.sequence_frame = ttk.Frame(section2)
        self.sequence_frame.pack(fill=tk.X, pady=2)
    ...
    def create_control_buttons(self, parent):
        ...
        # 更新按钮
        # ttk.Button(button_frame, text="Update Plots", command=self.update_plots,
        ...
        ttk.Button(control_frame, text="Reset View", command=self.reset_view,
                   style="Accent.TButton").pack(side=tk.TOP, fill=tk.X, pady=1)
        ...
        ttk.Button(zoom_frame, text="Zoom Out", command=self.zoom_out,
                   style="Accent.TButton").pack(side=tk.RIGHT, padx=2, fill=tk.X,
                                                expand=True)

        ttk.Button(control_frame, text="Pan Mode", command=self.enable_pan_mode,
                   style="Accent.TButton").pack(side=tk.TOP, fill=tk.X, pady=1)
    ...
    def create_data_info_text(self, parent):
        """创建数据信息显示"""
        section5 = ttk.LabelFrame(parent, text="Log", padding=10)
        section5.pack(fill=tk.X, padx=5, pady=5)

        self.data_info_text = tk.Text(section5, height=4, width=40, font=("TkDefaultFont", 10))
        self.data_info_text.pack(pady=(5, 0), fill=tk.X)
        self.data_info_text.insert(tk.END, "No dataset loaded")
        self.data_info_text.config(state=tk.DISABLED)
    ...
    def create_heatmap_controls(self, parent):
        ...
        ttk.Button(heatmap_frame, text="Load Heatmap Data (.npy)",
                   command=self.load_heatmap_data, style="Accent.TButton").pack(fill=tk.X, pady=2)
        ...
        ttk.Button(heatmap_frame, text="Update Heatmap",
                   command=self.update_heatmap, style="Large.TButton").pack(fill=tk.X, pady=10)
        ...
        self.click_info_label = ttk.Label(heatmap_frame, text="Click one point on heatmap to compare shapes",
                                          foreground="blue", font=("TkDefaultFont", 10))
    ...
    def create_attention_controls(self, parent):
        ...
        ttk.Button(attention_frame, text="Load Attention Data (.npy)",
                   command=self.load_attention_data, style="Accent.TButton").pack(fill=tk.X, pady=2)
        ...
        ttk.Button(attention_frame, text="Update Attention Plot",
                   command=self.update_attention_plot, style="Large.TButton").pack(fill=tk.X, pady=10)

        # 下载索引按钮
        ttk.Button(attention_frame, text="Export Top Shapes Indices",
                   command=self.download_indices, style="Accent.TButton").pack(fill=tk.X, pady=2)
    ...
    def update_heatmap(self):
        ...
        try:
            ...
            # 设置标题
            self.current_heatmap_ax.set_title(
                f'Heatmap: Instance {sample_idx + 1}, Shapes {start_shape + 1}-{end_shape}')
    ...
    def show_comparison_window(self, sample1_idx, shape1_idx, length1, start1, end1, label1,
        ...
        # 验证variable索引范围并转换为整数
        var1_idx = int(label1)
        var2_idx = int(label2)

        # 确保variable索引在有效范围内
        if var1_idx >= self.x_train.shape[2] or var1_idx < 0:
            var1_idx = 0
        if var2_idx >= self.x_train.shape[2] or var2_idx < 0:
            var2_idx = 0

        # 绘制第一组时间序列数据 - 使用正确的variable
        if start1 < self.x_train.shape[1] and end1 <= self.x_train.shape[1]:
            ts1 = self.x_train[sample1_idx, :, var1_idx]  # 使用正确的variable索引
